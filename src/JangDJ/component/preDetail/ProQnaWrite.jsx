...
function ProQnaWrite({product}) {
    const navigate = useNavigate();
    const { productId } = useParams(); // URL에서 productId를 받아옴
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // 상품에 대한 Q&A 등록 API 요청
            const response = await fetch(`http://localhost:8080/api/productqna/product/${productId}`, {
                method: 'POST',
                headers: {
            ...
            alert('게시글 등록 완료!');
            navigate(`/product/${productId}`); // 작성 후 해당 상품의 Q&A 목록으로 이동
        } catch (error) {
    ...
    const handleList = () => {
        navigate(`/product/${productId}`); // 수정된 부분
    };
